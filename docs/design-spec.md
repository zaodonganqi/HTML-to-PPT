...
> 用浏览器完成真实 Web 渲染、布局测量与 canvas 绘制信息采集，用 PPTX 完成交付；在保持高视觉保真的同时，让文本、表格和有限动画成为 PowerPoint 原生可编辑对象，非文本复杂视觉以 PPT 图片元素稳定承载。
...
HTP runtime 标注语义和动画
        ↓
HTP exporter 读取 DOM + CSSOM + manifest + layout + canvas pixels
        ↓
PPTX
```
...
### 2.2 为什么需要浏览器真实信息与 canvas 栅格化

PowerPoint 的原生对象体系和 HTML/CSS 不是同构的。CSS 的 grid、flex、filter、mask、blend、复杂 SVG、canvas、WebGL、Lottie 动画，都很难稳定转成 PowerPoint 原生对象。但 HTP 真正依赖浏览器的核心原因不是截图，而是获取浏览器完成布局和绘制后的真实信息：DOM 坐标、computed style、字体度量、图片尺寸、canvas 像素以及最终可见状态。

所以 HTP 应采用分层策略：

```text
可稳定映射的对象 -> PPT 原生对象
不可稳定映射的对象 -> 浏览器侧 canvas 栅格化 -> PPT 图片元素
```
...
text  -> PPT text box
table -> PPT table
image/canvas/SVG/CSS visual -> canvas rasterized PPT picture
其他复杂视觉 -> canvas rasterized PPT picture
```

这会显著降低实现难度，同时满足真实交付需求。浏览器在这里承担真实 layout 与绘制引擎职责，PPTX 写入层负责把采集到的信息转换为可交付的 Office Open XML 结构。
...
使用浏览器侧 canvas 栅格化生成非文本视觉的 PPT 图片元素
...
flowchart TD
  ...
  E --> G["@htp/exporter"]
  F --> G
  G --> H["Text/Table native extraction"]
  G --> I["Canvas-rasterized visual elements"]
  H --> J["PPTX writer"]
  I --> J
  J --> K["deck.pptx"]
```
...
1. 浏览器是真实布局与绘制引擎，不自己实现 CSS layout、字体度量或 canvas 绘制语义。
2. runtime 是语义层，不负责生成 PPTX。
3. exporter 是转换层，读取最终 DOM、CSSOM、布局坐标、computed style 与 canvas 像素信息。
4. 未定义或不可映射内容通过浏览器侧 canvas 栅格化为 PPT 图片元素。
...
```
节点合并策略用于控制父子标记节点的导出所有权。默认 `auto` 尽量保持调用简单：父节点栅格化时排除已标记子节点，文本、表格和图片子节点各自导出，避免重复绘制。

```ts
type MergeMode = "auto" | "text" | "all" | "none" | "only";
...
htp.image(".card", { merge: "text" }); // 文本和表格并入父级截图
htp.image(".poster", { merge: "all" }); // 子节点全部并入父级截图
htp.text(".badge", { merge: "only" });     // 只作为子节点边界，不单独导出当前节点
```
...
导出时不能太早采集渲染信息，否则图片、字体、图表、canvas 和异步数据可能还没渲染完。
...
浏览器能加载的字体不等于 PowerPoint/WPS 能稳定打开的字体。导出器需要在写入 PPTX 前解析 CSS `font-family`，并用项目配置把 Web 字体映射到 PPT 安全字体。
...
1. `table` 是允许直接写入 PPTX 的字体表，未提供时使用内置 Office/WPS 常见字体表。
2. `map` 由业务项目配置，例如把 `Inter` 映射到 `Arial`，把标题衬线字体映射到 `Times New Roman`。
3. 未命中字体表和映射规则时，按文本内容与默认配置选择 `defaultFont`、`eastAsia` 或 `monospace`。
4. `warn !== false` 时，导出器在控制台和 `warnings` 中报告 `HTP_FONT_NOT_IN_TABLE`。
5. 库默认值不能写死某个测试项目或业务项目的品牌字体。
...
sequenceDiagram
  ...
  Exporter->>Browser: read layout/style and rasterize non-text regions through canvas
...
1. 先处理 slide 背景图层。
2. 再处理 image/canvas/SVG/CSS 装饰等非文本图片元素。
3. 再处理 table 与 text 等可编辑对象。
4. 再处理 table。
5. 最后处理 text。
...
如果整页栅格化后再叠加可编辑文本，会出现文本重复。

推荐做法：

```text
栅格化前临时隐藏可编辑节点
通过浏览器侧 canvas 得到背景/复杂视觉图层
恢复节点
导出可编辑 text/table，并导出非文本 PPT 图片元素
```
...
const bg = await rasterizeElementToPng(slideElement);
...
```text
DOM node -> browser canvas rasterization -> PNG -> PPT picture element
```

`image` 不只表示 HTML `<img>` 标签，而是表示“按浏览器真实渲染结果导出为 PPT 图片元素”的视觉区域。它可以覆盖 `<img>`、canvas、SVG、CSS 装饰、图表容器和其他无法稳定原生映射的非文本视觉内容。实现上依赖 Chromium 的布局、绘制与像素读取能力，避免重新实现 CSS、SVG、canvas、伪元素和滤镜语义。
### 6.9.1 标记节点合并策略

当父节点和子节点都被标记时，导出器先建立标记节点树，再按 `merge` 决定父节点栅格化时是否隐藏子节点，以及子节点是否独立导出。
...
htp.image(".card");
htp.text(".card-title");
htp.table(".card-table");
htp.image(".card-chart");
```

导出结果：

```text
.card 未标记内容和装饰 -> PPT 图片元素
.card-title -> PPT 原生文本
.card-table -> PPT 原生表格
.card-chart -> PPT 图片元素
```
...
| merge | 父图包含 text/table | 父图包含 image 子节点 | 子 text/table 单独导出 | 子 image 单独导出 | 适用场景 |
|------|:-------------------:|:-----------------------:|:----------------------:|:---------------------------:|---------|
| `auto` | 否 | 否 | 是 | 是 | 默认；关键文字和表格保持可编辑 |
| `text` | 是 | 否 | 否 | 是 | 卡片整体视觉稳定，但图表/canvas 仍单独处理 |
| `all` | 是 | 是 | 否 | 否 | 海报、封面、复杂整块视觉一次性栅格化 |
| `none` | 否 | 否 | 否 | 否 | 只导出当前节点本身视觉，忽略已标记子节点 |
| `only` | 否 | 否 | 是 | 是 | 当前节点只作为分组边界，不导出父节点 |

```js
htp.image(".card", { merge: "text" });
```

上例中 `.card` 的栅格图会包含内部文本和表格，但已标记的 image 子节点仍从父节点中排除并单独导出。`text` 与 `table` 仍是清晰的原生对象标记；当它们需要并入父级视觉时，由父级 `image` 节点的 `merge` 策略决定。
...
2. `text` 的公开语义是原生可编辑文本，不承担栅格化开关。
3. 如果某段文字必须完全按浏览器视觉交付，应把承载它的 DOM 区域标记为 `image`，由浏览器渲染后作为 PPT 图片元素写入。
...
第一版可先把图片节点按浏览器最终渲染结果栅格化为 PPT 图片元素，而不是提取原图资源。这样可以自然包含裁剪、圆角、滤镜和叠加效果。
...
第一版也必须保持 Office/WPS 可直接打开，不能只追求“zip 结构存在”。当前生成器需要满足这些兼容性约束：

```text
presentation relationships must include slides, slide master, presProps, viewProps, theme, and tableStyles
[Content_Types].xml must include overrides for presentation, slides, master/layout, theme, presProps, viewProps, and tableStyles
slide master must include p:txStyles
slide layout and each slide must include p:clrMapOvr
background p:bgPr should include an effect list when emitted
paragraph alignment must be written on a:pPr@algn, using DrawingML tokens such as l, ctr, r, and just
table cells should avoid invalid anchor attributes and empty text runs
table grid rows/columns should include stable a16 rowId/colId extensions
```
...
blank master/layout
theme
presentation/view/table style parts
images
text boxes
tables
...
```text
DOM/CSSOM/layout/canvas pixels -> HTP PPTX object model
```
...
image 栅格图和原始 image/canvas/SVG 节点都可以走同一套 image part 写入逻辑。
...
```text
简单 table -> PPT table
复杂 table -> canvas rasterized PPT picture
```

### 13.3 图片与非文本视觉

节点按浏览器最终渲染结果栅格化优先，保证视觉。未来再支持原图嵌入和替换。

### 13.4 Shape

第一阶段不支持 shape。所有图形、卡片、背景块、SVG 默认通过浏览器侧 canvas 栅格化为 PPT 图片元素。
...
Playwright 打开页面
识别 slide
读取真实 DOM/CSSOM/layout/canvas 信息
生成单页/多页 PPTX
```
...
```text
table -> PPT table
img/canvas/SVG/image -> PPT picture through browser-side rasterization
image node -> PPT picture through browser-side rasterization
```
...
浏览器负责真实渲染、布局测量和 canvas 绘制信息采集
exporter 负责读取 DOM/CSSOM/layout/canvas 信息并生成 PPTX 对象模型
文本/表格/有限动画原生化
图片、canvas、SVG、CSS 装饰和复杂视觉通过 canvas 栅格化为 PPT 图片元素
```
...
> 任意 Web 页面可以稳定导出为 PPTX，其中关键文本、表格和有限动画是 PowerPoint 原生对象，其余图片、canvas、SVG、CSS 装饰和复杂视觉由浏览器真实渲染后通过 canvas 栅格化为 PPT 图片元素。
